...
client.on("message", message => {
 if (message.content === "-support") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter('© ARMY SYSTEAM جميع الحقوق محفوظة 2018 لــبوت')
      .addField('سيرفر الدعم الفني', `https://discord.gg/Tk6QubN`)
  message.author.send({embed});
      message.channel.send(":white_check_mark: | Check Your DM تم الأرسال بلخاص")
 }
});
...
if (message.content.startsWith('-bcall')){
 if (message.author.id !== '403641716797603853') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
 if(!message.author.id === '403641716797603853') return;
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .addField('``My ID``' , `[ ${client.user.id} ]` , true)
                  ...
                  .setFooter('By |<@403641716797603853>')
    })
}
});
...
client.on('guildCreate', guild => {
  client.channels.get("481224550655328256").send(`:white_check_mark: **تم اضافة البوت في سيرفر جديد مبروكك
...
client.on('guildDelete', guild => {
  client.channels.get("481224550655328256").send(`:negative_squared_cross_mark: **طردوني حرام والله ايش سويت انا
Server name: __${guild.name}__
Server owner: __${guild.owner}__
Server id: __${guild.id}__ 
Server Count: __${guild.memberCount}__**`)
...
👑Commands ARMY SYSTEAM♧|أوامر البوت الأسطورية👑
...
client.on("message", message => {
 if (message.content === "-elp-welcome") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
